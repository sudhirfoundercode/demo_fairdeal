...
{	
     ...
     public function auto_result_insert(Request $request){
         ...
         $game_sr_num=$games_no[0]->games_no;
           ...
           $a = DB::table('green_36admin_winner_result')->insert([
               'game_no'=>$game_sr_num,
               'number'=>$number,
               ]);
           if($a){
		...
		public function green36_result_store()
{
			...
			$gamesno=DB::table('green_36_bet_logs')->value('games_no');
			$admin_result = DB::table('green_36admin_winner_result')->where('game_no',$gamesno)->orderBy('id','desc')->value('number');
			...
			$number=$admin_result;
				...
				$results=DB::Select("SELECT * FROM `green_36_bet_logs` WHERE `number`=$number");
			...
			$result3=DB::Select("SELECT * FROM `green_36_bet_logs` ORDER BY `amount` ASC LIMIT 1");
				$number=$result3[0]->number;
				$game_idd=json_decode($result3[0]->game_id);
			//dd($number);
			}
			
			$bet_details= DB::select("SELECT * FROM `green_36_bets` WHERE `games_no`=$gamesno");
			...
			$multiplier=DB::table('green_36_game_settings')->where('game_id',$game_ids )->value('multiplier');
		   ...
		   DB::table('users')->where('id',$userid)->update(['wallet'=>DB::raw("wallet+$total_multy_amt")]);
			   DB::table('green_36_bets')->where('id',$bet_ids)->update(['win_amount'=>$total_multy_amt,'status'=>1,'win_number'=>$number]);
		   }else{
		    DB::table('green_36_bets')->where('id',$bet_ids)->update(['status'=>2,'win_number'=>$number]);

		   }
				
			}
	
		$red_black=DB::Select("SELECT * FROM `green_36_game_settings` WHERE number=$number && game_id IN(45,46)");
			if($red_black ){
		...
		if($game_ids == 45){
			...
			$status = 2;
			 ...
			 $number_index=DB::select("SELECT `index` FROM `green_36_wheel_index` WHERE value=$number");
		$index=$number_index[0]->index;
	     
		
					$store=DB::select("INSERT INTO `green_36_results`( `games_no`, `number`,`index`,`status`, `time`) VALUES ('$gamesno','$number','$index','$status','$formattedTime')");
		
			DB::table('green_36_bet_logs')->update(['amount'=>0,'games_no'=>DB::raw("games_no+1")]);
		 //$this->amount_distributation($gamesno);
      
}
	
	 
	public function green36_result_index()
	{
	
	$last_result=DB::select("SELECT * FROM `green_36_results` ORDER BY `id` DESC");
		$value=$last_result[0]->number;
		$games_no=$last_result[0]->games_no;
		$number_index=DB::select("SELECT `index` FROM `green_36_wheel_index` WHERE value=$value");
		$index=$number_index[0]->index;
	...
	public function green36_bet_history(Request $request)
	{
    ...
    if (!empty($userid)) {
        $where[] = "green_36_bets.user_id = '$userid'";
    }
    $query = "SELECT g.games_no AS games_no, g.number AS number, COALESCE(b.game_id, '') AS game_id, COALESCE(b.amount, '') AS amount ,COALESCE(b.win_amount, '') AS win_amount FROM green_36_results g LEFT JOIN green_36_bets b ON g.games_no = b.games_no ";
	...
	//$bet_history=DB::select("SELECT g.games_no AS games_no, g.number AS number, COALESCE(b.game_id, '') AS game_id, COALESCE(b.amount, '') AS amount,COALESCE(b.win_amount, '') AS win_amount FROM green_36_results g LEFT JOIN bets b ON g.games_no = b.games_no AND b.user_id = 1 ORDER BY g.games_no DESC LIMIT 10");
		$result_count=DB::select("SELECT COUNT(`id`) as id FROM `green_36_results`");
	...
	public function green36_last13_result()
	{
...
//$last_result = DB::select("SELECT * FROM green_36_results ORDER BY id DESC LIMIT 12 OFFSET 1");
	...
	public function green36_bet(Request $request)
{
		$kolkataTime = Carbon::now('Asia/Kolkata');
...
$formattedTime = $kolkataTime->toDateTimeString();
   ...
   // dd($gamesno);
    ...
    foreach ($testData as $item) {
      ...
      if ($amount>=1) {
        DB::insert("INSERT INTO `green_36_bets`(`user_id`, `game_id`, `amount`, `games_no`, `status`, `created_at`, `updated_at`) VALUES ('$userid','$gameid','$amount','$gamesno','0','$formattedTime','$formattedTime')");
            DB::table('users')->where('id', $userid)->update(['wallet' => DB::raw('wallet - ' . $amount)]);
      }
			$multiplier=DB::table('green_36_game_settings')->where('game_id',$gameid )->value('multiplier');
			
			$bet_log = DB::select("SELECT * FROM green_36_bet_logs ");
            ...
            $multiply_amt = $amount * $multiplier;
				if(in_array($gameid, $game_id_array)) {
                     $bet_amt= DB::update("UPDATE `green_36_bet_logs` SET `amount`=amount+'$multiply_amt' where number= $num");
                    }
             }
	...
	public function green36_win_amount(Request $request)
      {
		  $validator = Validator::make($request->all(), [
	...
	public function getLatestBetLogsAmount()
{
    ...
    $betLogs = DB::table('green_36_bet_logs')
 ...
 public function getLatestBetLogs()
{
    ...
    $latestBetLogs = DB::table('green_36_bet_logs')
                        ->select('games_no')
...
public function admin_prediction3(Request $request){
       ...
       // dd($custom_date_time);
          ...
          $prediction_insert = DB::table('green_36admin_winner_result')->insert(['card_number'=>$number,'result_time'=>$result_time ?? now(),
          'games_no' => $game_no
          ]);
